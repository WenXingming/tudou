...
void Connection::handle_read(EventLoop& loop) {
    ...
    // 更改监听事件 EPOLLOUT（回声客户端...），将业务层结果发送给客户端
    epoll_event ev{};
    ev.data.fd = fd;
    ev.events = EPOLLOUT;
...
ssize_t Connection::read_from_fd() {
    ...
    while (true) { // 边缘触发（ET）模式，那更必须用循环，否则会丢事件；水平模式也用，否则剩下的数据要等下一轮 epoll 事件才会读，导致延迟增加，尤其是大消息/高吞吐场景
        ...
        if (n > 0) {
            ...
            if (errno == EAGAIN || errno == EWOULDBLOCK) { // 内核缓冲区为空，（暂时）已读完
                // return totalSize;
                break;
            }
            else return -1; // 其它错误
...
/// @brief 把 writeBuffer 数据写入 fd，内核自动将 fd 数据发送给客户端
/// @return 写入的字节数。0 表示未写入，-1 表示错误
ssize_t Connection::write_to_fd() {
    ssize_t totalSize = 0;

    while (!writeBuffer.empty()) {
        ssize_t n = ::send(fd, writeBuffer.data() + totalSize, writeBuffer.size() - totalSize, 0); // 是不是发太大了...是否应该分批次写入 fd?
        if (n > 0) {
            totalSize += n;
        }
        else {
