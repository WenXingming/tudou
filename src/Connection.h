...
class Connection {
	...
	std::function<void(Connection&)> read_cb;
	std::function<void(Connection&)> write_cb;
	// std::function<void(Connection&)> accept_cb;
	std::function<void(EventLoop&, Connection&)> accept_cb;


public:
	explicit Connection(int _fd = -1, bool _isListenFd = false);
	~Connection();
